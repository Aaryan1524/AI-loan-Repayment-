      ...
      <main className="flex-1 px-4 py-6 md:px-9 md:py-8 pb-24 md:pb-8 overflow-y-auto">
        <div className="max-w-[900px]">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6 md:mb-8">
            <div>
              <h1 className="text-2xl md:text-4xl font-bold mb-1 md:mb-2">My Loans</h1>
          ...
          {loans.length === 0 ? (
            <div className="bg-surface-light border border-border-light rounded-[20px] p-6 md:p-12 text-center">
              ...
              <h3 className="text-lg md:text-xl font-medium mb-2">No loans yet</h3>
              <p className="text-sm md:text-base text-text-muted-light mb-6">
                Add your first loan to start tracking your repayment journey
              </p>
              <button
                onClick={handleAdd}
                className="inline-flex items-center justify-center gap-2 bg-primary text-white px-6 h-12 md:h-10 rounded-xl font-medium hover:bg-opacity-90 transition-opacity text-sm md:text-base w-full md:w-auto"
              >
                <Plus size={18} />
                Add your first loan
              </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
              {loans.map((loan) => {
                ...
                return (
                  <div
                    key={loan.id}
                    className="bg-surface-light border border-border-light rounded-[20px] p-5 md:p-6 flex flex-col justify-between hover:shadow-md transition-shadow group"
                  >
                    {/* Top row: icon + info */}
                    <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-5">
                      <div
                        className={`w-10 h-10 md:w-12 md:h-12 rounded-full ${style.bg} flex items-center justify-center ${style.fg} flex-shrink-0`}
                      >
                        {style.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="font-medium text-base md:text-lg truncate">{loan.name}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4 md:mb-5">
                      <div>
                        ...
                        <div className="text-lg md:text-xl font-bold">{formatCurrency(loan.balance, currency)}</div>
                      </div>
                      <div>
                        <div className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-text-muted-light mb-1">
                          Principal
                    ...
                    <div className="flex gap-2 pt-4 border-t border-border-light">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 px-4 h-10 md:h-10 rounded-xl border border-border-light text-text-main-light text-xs md:text-sm font-medium hover:bg-background-light transition-colors"
                      >
                        <Pencil size={14} />
                        Edit
                      </button>
                      <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-1.5 px-4 h-10 md:h-10 rounded-xl text-xs md:text-sm font-medium transition-colors ${
                          isConfirmingDelete
              ...
              <button
                onClick={handleAdd}
                className="border-2 border-dashed border-border-light rounded-[20px] p-5 md:p-6 flex flex-col items-center justify-center gap-3 min-h-[240px] md:min-h-[280px] hover:border-primary hover:bg-[#faf6f0] transition-colors group"
              >
