    ...
    <div className="bg-background-light dark:bg-background-dark text-text-main-light dark:text-text-main-dark min-h-screen flex">
      ...
      <main className="flex-1 p-10 overflow-y-auto relative">
        <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 auto-rows-max lg:auto-rows-min">
            {/* Top Summary Cards */}
            <div className="lg:col-span-8 flex flex-col gap-6">
              {/* Total Debt & Payoff Date Row */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Total Debt */}
                <div className="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-[20px] p-6 flex flex-col justify-between">
                  <div>
                    <div className="text-sm font-bold tracking-wider text-text-muted-light dark:text-text-muted-dark uppercase mb-2">
                      Total Debt
                    </div>
                ...
                <div className="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-[20px] p-6 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="font-display text-4xl font-bold leading-tight mb-2">
                      {payoffMonth}<br />{payoffYear}
                    </div>
                    {interestSaved > 0 && (
                      <div className="text-sage font-medium text-sm">
                        ↓ Saving {formatCurrency(interestSaved, currency)} with optimization
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-primary rounded-[20px] p-6 flex flex-col justify-between text-white shadow-sm">
                <div>
                  <div className="text-sm font-bold tracking-wider uppercase mb-2 opacity-90">
                    Interest<br />You Can<br />Save
                  </div>
            ...
            <div className="lg:col-span-4 lg:row-start-1 lg:row-span-2">
              {/* ─── AI Insight Card ─── */}
              <div className="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-[20px] p-6">
                ...
                {adviceLoading ? (
                  ...
                  <>
                    <p className="text-lg leading-relaxed text-text-main-light dark:text-text-main-dark mb-4">
                      {advice.insight}
                    </p>
                    ...
                    <div className="flex gap-4">
                      <button className="bg-primary text-white px-6 py-2.5 rounded-xl font-medium hover:bg-opacity-90 transition-opacity">
                        Apply suggestions
                      </button>
                      <button
                        onClick={fetchAdvice}
                        className="border border-border-light dark:border-border-dark text-text-main-light dark:text-text-main-dark px-6 py-2.5 rounded-xl font-medium hover:bg-background-light dark:hover:bg-background-dark transition-colors"
                      >
                        Refresh insight
                      </button>
                    </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-5 bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-[20px] p-6">
              <h3 className="text-xl font-medium mb-6">Your loans</h3>
              ...
              {loans.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <Link
                    href="/loans"
                    className="inline-flex items-center gap-2 bg-primary text-white px-5 py-2.5 rounded-xl font-medium hover:bg-opacity-90 transition-opacity text-sm"
                  >
                    <Plus size={16} />
                    Add your first loan
                  </Link>
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between ${
                          !isLast
                            ? "pb-6 border-b border-border-light dark:border-border-dark"
                            : ""
                        }`}
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-12 h-12 rounded-full ${style.bg} flex items-center justify-center ${style.fg}`}
                          >
                            {style.icon}
                          </div>
                          <div>
                            <div className="font-medium">{loan.name}</div>
                            <div className="text-sm text-text-muted-light dark:text-text-muted-dark mt-0.5">
                              {loan.rate}% · {formatTerm(loan.termMonths)}
                            </div>
                          </div>
                        </div>
                        <div className="font-medium">{formatCurrency(loan.balance, currency)}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="lg:col-span-7 bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-[20px] p-6 flex flex-col">
              <h3 className="text-xl font-medium mb-6">Payoff timeline comparison</h3>
              <div className="flex gap-2 mb-8">
                <button className="px-4 py-1.5 border border-border-light dark:border-border-dark rounded-full text-sm font-medium hover:bg-background-light dark:hover:bg-background-dark transition-colors">
                  Monthly
                </button>
                <button className="px-4 py-1.5 bg-background-light dark:bg-background-dark border border-transparent rounded-full text-sm font-medium">
                  Quarterly
                </button>
                <button className="px-4 py-1.5 border border-border-light dark:border-border-dark rounded-full text-sm font-medium hover:bg-background-light dark:hover:bg-background-dark transition-colors">
                  Yearly
              ...
              <div className="flex-1 relative flex items-end min-h-[200px] mb-6">
                <div className="w-full flex items-end justify-between gap-2 h-full pb-8 border-b border-border-light dark:border-border-dark relative">
                  ...
                  {timelineBars.map((bar, i) => (
                    <div
                      key={i}
                      className="w-1/12 bg-terra-light dark:bg-[#4a3625] rounded-t-md relative z-10"
              ...
              <div className="flex gap-6 mt-auto">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-terra-light dark:bg-[#4a3625]" />
                  <span className="text-sm text-text-muted-light dark:text-text-muted-dark">Baseline</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-primary" />
                  <span className="text-sm text-text-muted-light dark:text-text-muted-dark">Optimized</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <button
          ...
          className="fixed bottom-8 right-8 w-14 h-14 bg-primary text-white rounded-full shadow-lg flex items-center justify-center hover:bg-opacity-90 transition-all hover:scale-105 z-30"
          title="Add loan"
        >
          <Plus size={24} />
        </button>
